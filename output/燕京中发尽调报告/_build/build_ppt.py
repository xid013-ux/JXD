...
label(1.0, 1.20, 24, 0.55, '（土地事项按方案一「收购燕京啤酒（邢台）有限公司100%股权」列示；本图按股权支付方式绘制，若采用现金支付则发行人股权结构不变）', sz=9.5, color=RGBColor(0x88,0x88,0x88))
...
label(14.45, 8.4, 4.9, 5.4,
      '调整步骤\n'
      '1、发行人收购燕京啤酒持有的中发邢台 67% 股权\n'
      '2、发行人收购燕京啤酒持有的燕京啤酒（邢台）100% 股权\n'
      '   （上述两步的对价均可选股权支付或现金支付，宜统一安排）\n'
...
label(19.4, 13.9, 13.5, 2.6,
      ...
      '③ 采用股权支付的，股权支付比例 100%，可争取企业所得税特殊性税务处理，预计仅产生印花税；\n'
      '   采用现金支付的，转让方按转让所得适用 25% 企业所得税，且发行人需实际支出资金。',
      sz=9, color=RGBColor(0x1B,0x7A,0x3D))

# ================= 页脚 =================
foot = s.shapes.add_connector(MSO_CONNECTOR.STRAIGHT, Cm(1.0), Cm(17.3), Cm(32.6), Cm(17.3))
foot.line.color.rgb = RGBColor(0xCC,0xCC,0xCC); foot.line.width = Pt(0.75)
label(1.0, 17.42, 24.5, 1.5,
      '注1：本图按股权支付方式绘制，发行人股东持股比例的变动仅发生于该方式下；若对价采用现金支付，'
      '发行人注册资本及股权结构不变，三名股东维持 80%／17.5%／2.5%，图中「↑上升／↓稀释」不适用。\n'
      '注2：土地事项本图列示方案一（收购100%股权）。若采用方案二（收购土地使用权及地上建筑物），'
      '燕京啤酒（邢台）有限公司仍由燕京啤酒持有，仅土地、厂房过户至上市主体，宗地四需另行以协议方式安排使用。\n'
      '注3：调整后持股比例以经国资监管部门备案的评估结果为准，本图为方案示意。',
      sz=8, color=RGBColor(0x88,0x88,0x88))
label(26.0, 17.42, 6.6, 0.9, '国泰海通证券  燕京中发IPO项目组', sz=8,
